...
class DBase::MySQL extends DBase::Main {
...
#### LOAD DATA FROM .TSV FILE INTO TABLE
	...
	if ( not $count ) {   print "DBase::MySQL    DID NOT LOAD!\n";    }
	$self->dbh()->{RaiseError} = 1; # ENABLE TERMINATION ON ERROR
}
...
print "DBase::MySQL::useDatabase    database: $database\n";
...
#### RETURN 1 IF A DATABASE EXISTS WITH THE GIVEN NAME, OTHERWISE RETURN 0
	...
	die "DBase::MySQL::isDatabase    user not defined\n" if not defined $user;
	die "DBase::MySQL::isDatabase    password not defined\n" if not defined $password;
